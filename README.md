...
* **替 (正则替换)**：内置针对原文标点纠偏的固定规则，用于自动字号计算（如将 `！！！` 替换为双惊叹号 `‼`）。
* **自 (自动字号/行距)**：
* **FontSize自**：基于 100px 测量基准，根据气泡物理空间反向推算最佳字号。
* **LineSpacing自**：基于 OCR 原始坐标投影，还原漫画原稿的行间比例。
...
* **查找替换译文**
* **调色盘**：可以吸取或选择颜色 ，用于局部颜色富文本[C]的辅助工具 
...
| **Shift + 点击** | 多选模式：用于合并框体或批量删除 |
| **Alt + 左键拖动画线** | 画线多选模式：用于合并框体或批量删除 |
---
